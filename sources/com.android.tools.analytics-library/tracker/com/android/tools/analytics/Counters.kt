...
object Counters {
  ...
  @JvmStatic
  fun getAll(): Collection<Counter> = synchronized(counters) { counters.values.toList() }

  // Gets or creates a new [Counter] named [name].
  @JvmStatic
  fun get(name: String): Counter = synchronized(counters) { counters.getOrPut(name) { Counter(name) } }
}
 ...
 * - the total time spent executing the measured code.
 * Time is measured as both wall and cpu time.
 */
